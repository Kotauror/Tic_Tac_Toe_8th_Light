...
describe Board do
  ...
  context 'Game in progress' do
    ...
    describe '#game_is_over?' do
      it 'tells that the game is over when 1-2-3' do
        board.put_sign_on_board(active_player, "1")
        board.put_sign_on_board(active_player, "2")
        board.put_sign_on_board(active_player, "3")
        expect(board.game_is_over?).to eq(true)
      end
      it 'tells that the game is over when 4-5-6' do
        board.put_sign_on_board(active_player, "4")
        board.put_sign_on_board(active_player, "5")
        board.put_sign_on_board(active_player, "6")
        expect(board.game_is_over?).to eq(true)
      end
      it 'tells that the game is over when 7-8-9' do
        board.put_sign_on_board(active_player, "7")
        board.put_sign_on_board(active_player, "8")
        board.put_sign_on_board(active_player, "9")
        expect(board.game_is_over?).to eq(true)
      end
      it 'tells that the game is over when 1-4-7' do
        board.put_sign_on_board(active_player, "1")
        board.put_sign_on_board(active_player, "4")
        board.put_sign_on_board(active_player, "7")
        expect(board.game_is_over?).to eq(true)
      end
      it 'tells that the game is over when 2-5-8' do
        board.put_sign_on_board(active_player, "2")
        board.put_sign_on_board(active_player, "5")
        board.put_sign_on_board(active_player, "8")
        expect(board.game_is_over?).to eq(true)
      end
      it 'tells that the game is over when 3-6-9' do
        board.put_sign_on_board(active_player, "3")
        board.put_sign_on_board(active_player, "6")
        board.put_sign_on_board(active_player, "9")
        expect(board.game_is_over?).to eq(true)
      end
      it 'tells that the game is over when 1-5-9' do
        board.put_sign_on_board(active_player, "1")
        board.put_sign_on_board(active_player, "5")
        board.put_sign_on_board(active_player, "9")
        expect(board.game_is_over?).to eq(true)
      end
      it 'tells that the game is over when 3-5-7' do
        board.put_sign_on_board(active_player, "3")
        board.put_sign_on_board(active_player, "5")
        board.put_sign_on_board(active_player, "7")
        expect(board.game_is_over?).to eq(true)
      end
      it 'tells that the game is not over - other cases' do
        board.put_sign_on_board(active_player, "1")
        board.put_sign_on_board(active_player, "5")
        board.put_sign_on_board(active_player, "7")
        expect(board.game_is_over?).to eq(false)
      end
    end
  end
